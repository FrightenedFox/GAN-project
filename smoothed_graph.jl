...
# ╔═╡ f2878ab3-3708-4e4e-88ea-43b8243a042d
begin
	using PlutoUI
	using DataFrames
	using Plots
...
function moving_average(vs, n)
	...
	gate = div(n, 2)
	out_vect = zeros(length(vs))
	for i in 1:length(vs)
		if i <= gate || i > length(vs) - gate
			out_vect[i] = NaN64
		else
			out_vect[i] = sum(skipmissing(@view vs[(i-gate):(i+gate)]))/n
		end
	end
	return out_vect
end
...
begin
	df_list = [DataFrame() for _ in 1:length(files)]
	for (i, file) in enumerate(files)
		df_list[i] = DataFrame(CSV.File(folder * file));
	end
	combined_df = leftjoin(df_list[1], df_list[2], on = :batch, renamecols = "4" => "8")
	for i in 3:length(df_list)
		combined_df = leftjoin(combined_df, df_list[i], on = :batch, renamecols = "" => "$(i*4)")
	end
	df_list = nothing
	sort!(combined_df, [:batch])
	combined_df = coalesce.(combined_df, NaN)
end
...
begin
	smoothing_slider = @bind smoothing PlutoUI.Slider(10:40:2000)
	gen_checkbox = @bind gen_check html"<input type=checkbox >"
	disc_checkbox = @bind disc_check html"<input type=checkbox >"
	
	md"""
	
	**Smoothing level:** $(smoothing_slider)
	
	**Generator losses:** $(gen_checkbox)
	
	**Discriminator losses:** $(disc_checkbox)
...
begin
	p = @df combined_df plot(
		:batch, 
		moving_average(:g_loss4, smoothing),
	...
	if gen_check
		for i in 8:4:80
			@df combined_df plot!(
				p, 
				:batch, 
				moving_average(combined_df[!, "g_loss$(i)"], smoothing), 
				label = "G$(i/4)"
			)
		end
	end
	if disc_check
		for i in 4:4:80
			@df combined_df plot!(
				p, 
				:batch, 
				moving_average(combined_df[!, "d_loss$(i)"], smoothing), 
				label = "G$(i/4)"
			)
		end
	end
